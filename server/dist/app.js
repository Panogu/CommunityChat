    ...
    connection.on('message', function (incoming) {
        ...
        if (incoming.type == "utf8") {
            ...
            if (incoming_parsed.type == 'register_user') {
                ...
                connection.sendUTF(JSON.stringify({ type: 'users_online', content: JSON.stringify(persons) }));
            }
                ...
                if (password == masterPassword) {
                    connection.sendUTF(JSON.stringify({ type: 'login_accepted', content: '' }));
                    console.log((new Date()) + " Login accepted!");
                }
                else {
                    connection.sendUTF(JSON.stringify({ type: 'login_rejected', content: '' }));
